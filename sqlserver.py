...
sqlalchemy_connection_string = pyodbc.connect(';'.join([f'{key}={value}' for key, value in db_config.items()]))
...
@app.before_request
def before_request():
    g.connection = pyodbc.connect(**db_config)
...
def login():
    ...
    if user and user[2] == password:
        return jsonify({'status': '200', 'user_id': user[0], 'identity': user[3]}), 200
    else:
...
def register():
    ...
    with g.connection.cursor() as cursor:
        # 执行插入
        insert_query = "INSERT INTO tb_user (username, password, [identity]) VALUES (?, ?, ?)"
        cursor.execute(insert_query, (username, password, identity))

        # 获取刚插入行的标识符
        select_query = "SELECT id as new_user_id FROM tb_user WHERE username = ?"
...
def update_password():
    ...
    try:
        with g.connection.cursor() as cursor:
            update_password_query = "UPDATE tb_user SET password=? WHERE id=?"
...
def delete_user():
    ...
    try:
        with g.connection.cursor() as cursor:
            delete_clickthroughrate_query = "DELETE FROM clickthroughrate WHERE user_id = ?"
            cursor.execute(delete_clickthroughrate_query, (user_id,))

            delete_collect_query = "DELETE FROM collect WHERE user_id = ?"
            cursor.execute(delete_collect_query, (user_id,))

            delete_comment_reply_query = "DELETE FROM comment_reply WHERE user_id = ?"
            cursor.execute(delete_comment_reply_query, (user_id,))

            delete_comment_query = "DELETE FROM comment WHERE user_id = ?"
            cursor.execute(delete_comment_query, (user_id,))

            delete_annex_query = "DELETE FROM annex WHERE user_id = ?"
            cursor.execute(delete_annex_query, (user_id,))

            delete_blog_query = "DELETE FROM blog WHERE user_id = ?"
            cursor.execute(delete_blog_query, (user_id,))

            delete_user_integral_query = "DELETE FROM user_integral WHERE user_id = ?"
            cursor.execute(delete_user_integral_query, (user_id,))

            delete_concern_query = "DELETE FROM concern WHERE user_id_2 = ?"
...
def check_user():
    ...
    try:
        with g.connection.cursor() as cursor:
            # 查询用户是否存在
            query = "SELECT id FROM tb_user WHERE username = ?"
            cursor.execute(query, (username,))
            user = cursor.fetchone()
...
# 模糊查看用户的接口
@app.route('/get_like_users', methods=['POST'])
def get_like_users():
    data = request.get_json()
...
def publish_article():
    ...
    with g.connection.cursor() as cursor:
        insert_blog_query = "INSERT INTO blog (title1, title2, title, user_id, creat_time, text) VALUES (?, ?, ?, ?, ?, ?)"
        ...
        if annex_route == 'None':
            ...
            insert_annex_query = "INSERT INTO annex (route, blog_id, user_id) VALUES (?, ?, ?)"
...
def edit_blog():
    ...
    with g.connection.cursor() as cursor:
        update_query = "UPDATE blog SET title1=?, title2=?, creat_time=?, user_id=?, text=?, title=? WHERE id=?"
...
def delete_blog():
    ...
    try:
        with g.connection.cursor() as cursor:
            delete_comments_query = "DELETE FROM comment WHERE blog_id = ?"
            cursor.execute(delete_comments_query, (blog_id,))

            delete_comment_replies_query = "DELETE FROM comment_reply WHERE blog_id = ?"
            cursor.execute(delete_comment_replies_query, (blog_id,))

            delete_annex_query = "DELETE FROM annex WHERE blog_id = ?"
            cursor.execute(delete_annex_query, (blog_id,))

            delete_clickthroughrate_query = "DELETE FROM clickthroughrate WHERE blog_id = ?"
            cursor.execute(delete_clickthroughrate_query, (blog_id,))

            delete_collect_query = "DELETE FROM collect WHERE blog_id = ?"
            cursor.execute(delete_collect_query, (blog_id,))

            delete_blog_query = "DELETE FROM blog WHERE id = ?"
...
def read_article():
    ...
    with g.connection.cursor() as cursor:
        query = "SELECT id, title, creat_time FROM blog WHERE title1 = ? AND title2 = ?"
...
def latest_articles():
    with g.connection.cursor() as cursor:
        query = "SELECT TOP 15 title, creat_time FROM blog ORDER BY creat_time DESC"
        cursor.execute(query)
        articles = cursor.fetchall()
...
def get_like_articles():
    ...
    with g.connection.cursor() as cursor:
        query = """
            ...
            WHERE b.title1 LIKE ? OR b.title2 LIKE ? OR b.title LIKE ? OR u.username LIKE ? OR b.text LIKE ?
        """
        cursor.execute(query, (
...
def get_eight_clicked_articles():
    try:
        with g.connection.cursor() as cursor:
            # 执行查询，合并相同文章的点击率
            sql = """
                SELECT TOP 8 b.id AS article_id, b.creat_time AS publish_time, b.title, SUM(c.click) AS total_click
                FROM blog b
...
def top8_collected_articles():
    try:
        with g.connection.cursor() as cursor:
            # 查询前八篇文章的收藏数量
            query = """
                SELECT TOP 8 b.id as article_id, b.creat_time, b.title, COUNT(c.id) as collect_count
                FROM blog b
...
def unfollow_user():
    ...
    try:
        with g.connection.cursor() as cursor:
            # 删除关注关系
            delete_query = "DELETE FROM concern WHERE user_id_1 = ? AND user_id_2 = ?"
...
def download_annex():
    ...
    # 根据blog_id查询附件数据
    with g.connection.cursor() as cursor:
        select_annex_query = "SELECT route FROM annex WHERE blog_id = ?"
...
def delete_annex():
    ...
    try:
        with g.connection.cursor() as cursor:
            # 删除附件表中对应的记录
            delete_query = "DELETE FROM annex WHERE id = ?"
...
def add_comment():
    ...
    try:
        with g.connection.cursor() as cursor:
            # 插入数据到评论表
            insert_query = "INSERT INTO comment (text, create_time, blog_id, user_id) VALUES (?, ?, ?, ?)"
...
def get_comments():
    ...
    try:
        with g.connection.cursor() as cursor:
            # 查询评论信息
            query = "SELECT id, text, create_time FROM comment WHERE blog_id = ? AND user_id = ?"
            cursor.execute(query, (blog_id, user_id))
            comments = cursor.fetchall()
...
def get_comments_g():
    ...
    try:
        with g.connection.cursor() as cursor:
            # 查询评论信息
            query = "SELECT id, text, create_time FROM comment WHERE user_id = ?"
            cursor.execute(query, user_id)
            comments = cursor.fetchall()
...
def delete_comment():
    ...
    try:
        with g.connection.cursor() as cursor:
            # 删除评论回复表中相关联的记录
            delete_reply_query = "DELETE FROM comment_reply WHERE comment_id = ?"
...
def add_comment_reply():
    ...
    try:
        with g.connection.cursor() as cursor:
            # 插入数据到回复评论表
            insert_query = "INSERT INTO comment_reply (text, create_time, blog_id, user_id, comment_id) VALUES (?, ?, ?, ?, ?)"
...
def get_comment_replies():
    ...
    try:
        with g.connection.cursor() as cursor:
            # 查询回复评论信息
            query = "SELECT id, text FROM comment_reply WHERE blog_id = ? AND comment_id = ?"
...
def modify_comment():
    ...
    try:
        with g.connection.cursor() as cursor:
            ...
            update_query = "UPDATE comment SET text=?, create_time=? WHERE id=?"
    ...
    with g.connection.cursor() as cursor:
        sql = """
            SELECT
                u.id AS user_id,
                u.username,
                ISNULL(COUNT(b.id), 0) AS article_count
            FROM
                tb_user u
            LEFT JOIN
    ...
    with g.connection.cursor() as cursor:
        query = """
            SELECT blog.id , blog.title1, blog.title2,
                   blog.title, tb_user.username, blog.create_time
            FROM collect
